...
// ### GLOBAL VARIABLES ###
int int_player_id = -1;
char g_identity[32] = {0};

dds_entity_t g_leave_writer = 0;
...
void *worker(void *arg)
{
	...
	while (1)
	{
		...
		if ((rc > 0) && (infos_position[0].valid_data))
		{
			...
			pthread_mutex_lock(&mutex);
			arr_Players[int_position_player_id - 1].b_active =
				position->b_active;

			if (position->b_active)
			{
				arr_Players[int_position_player_id - 1].x =
					position->int_x;

				arr_Players[int_position_player_id - 1].y =
					position->int_y;

				strncpy(
					arr_Players[int_position_player_id - 1].str_name,
					position->str_name,
					MAX_NAME_LEN);

				arr_Players[int_position_player_id - 1]
					.str_name[MAX_NAME_LEN] = '\0';
			}
			else
			{
				arr_Players[int_position_player_id - 1]
...
int main(int argc, char *argv[])
{
	...
	snprintf(g_identity, sizeof(g_identity), "client%s", str_client_id);
